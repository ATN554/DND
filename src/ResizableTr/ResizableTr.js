...
export default class ResizableTr extends React.Component {
  constructor(props) {
    ...
    let innerThStyle = {overflow: "hidden", display: 'inline-block', verticalAlign: 'middle'};
    ...
    let modElements = React.Children.map(elements, (childTh, idx) => {
      return React.cloneElement(
        childTh,
        { ...childTh.props, style: { overflow: "hidden", whiteSpace: "nowrap" } },
        [
  ...
  onDragStart(idFrom, x, y) {
    ...
    if (!curCol.style.minWidth) {
      curCol.style.minWidth = (this.props.minWidth ? this.props.minWidth : width) + "px";
    }
    this.setState({
      curCol: curCol,
      pageX: x,
